...
def getHistPercentilePos(hist,percentile=10):
    ...
    # pos = 0
    # while cumHistProb[pos] < prob:
    #     pos += 1
    return pos

def identifySignificantPeaks(hist):
    pmin = getHistPercentilePos(hist,percentile=1)
    pmax = getHistPercentilePos(hist,percentile=99)
    ...
    mu = np.mean(peakCounts)
    sigma = np.std(peakCounts)
    T = 0.5*sigma
    if T < 0.:
        return peaks
    else:
...
def calcDataRange(img,medFiltRangePx=1,plotHistChange=False):
    ...
    hist, edges = np.histogram(img[~np.isnan(img)], bins=256)
    vals = 0.5*(edges[1:] + edges[:-1]) # bin centers
    histMed,edgesMed = np.histogram(imgMed,bins=256)
    valsMed = 0.5*(edgesMed[1:] + edgesMed[:-1]) # bin centers after median filter
    sigma = len(histMed)/100.
